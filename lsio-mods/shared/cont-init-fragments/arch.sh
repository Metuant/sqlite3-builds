  ...
  case "$machine" in
    x86_64)
      ...
      if [ -r "$cpuinfo_path" ]; then
        flags="$(awk -F: '/^flags[[:space:]]*:/{print $2; exit}' "$cpuinfo_path")"
      fi
      has_all_flags() {
        for f in "$@"; do
      ...
      if has_all_flags cx16 lahf_lm popcnt pni sse4_1 sse4_2 ssse3 avx avx2 bmi1 bmi2 f16c fma abm movbe xsave; then
