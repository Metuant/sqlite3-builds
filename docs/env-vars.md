...
| `SQLITE3_RUNTIME_OPTIMIZE_LIMITED_SECONDS` | numeric tunable | LIMITED optimize cadence per target path. | `1800` seconds. | tunable | Positive unsigned decimal seconds within the source bound. | Unset, empty, `0`, signed, non-decimal, trailing junk, or overflow values use `1800`. | generic and Plex-ICU shared libraries; current deployments are Emby and Plex. | Applies only when runtime optimize is enabled. The cadence gates from the later of the last LIMITED success and the last LIMITED deadline-interrupted attempt. |
| `SQLITE3_RUNTIME_OPTIMIZE_FULL_SECONDS` | numeric tunable | FULL optimize cadence per target path. | `86400` seconds. | tunable | Positive unsigned decimal seconds within the source bound. | Unset, empty, `0`, signed, non-decimal, trailing junk, or overflow values use `86400`. | generic and Plex-ICU shared libraries; current deployments are Emby and Plex. | Applies only when runtime optimize is enabled. A successful FULL pass also satisfies the LIMITED cadence. The cadence gates from the later of the last FULL success and the last FULL deadline-interrupted attempt, so a FULL pass that cannot finish inside its deadline retries once per cadence, not once per failure backoff. |
| `SQLITE3_DISABLE_RUNTIME_OPTIMIZE_FULL` | kill-switch | Disable only the FULL runtime optimize tier. | FULL tier enabled (when runtime optimize is enabled). | opt-out | Literal `1` disables the FULL tier; the LIMITED tier keeps running. | Unset, literal `0`, and any other value keep the FULL tier enabled. | generic and Plex-ICU shared libraries; current deployments are Emby and Plex. | For databases where an unbounded `ANALYZE` can never finish inside the FULL deadline and offline maintenance owns full statistics. Consulted in both the due computation and tier reservation. |
| `SQLITE3_RUNTIME_OPTIMIZE_FULL_DEADLINE_MS` | numeric tunable | FULL tier wall-clock deadline. | `15000` ms. | tunable | Positive unsigned decimal milliseconds; values above `600000` clamp to `600000`. | Unset, empty, `0`, signed, non-decimal, or trailing-junk values use `15000`. | generic and Plex-ICU shared libraries; current deployments are Emby and Plex. | A run interrupted by this deadline records a deadline-attempt stamp and re-arms on the FULL cadence. Exists for deterministic testing and field tuning. |
| `SQLITE3_RUNTIME_OPTIMIZE_LIMITED_DEADLINE_MS` | numeric tunable | LIMITED tier wall-clock deadline. | `3000` ms. | tunable | Positive unsigned decimal milliseconds; values above `600000` clamp to `600000`. | Unset, empty, `0`, signed, non-decimal, or trailing-junk values use `3000`. | generic and Plex-ICU shared libraries; current deployments are Emby and Plex. | A run interrupted by this deadline records a deadline-attempt stamp and re-arms on the LIMITED cadence. Exists for deterministic testing and field tuning. |
...
| `SQLITE3_DISABLE_EMBY_FANOUT_REWRITE` | kill-switch | Control Emby fan-out rewrites: Browse-by-name, Favorites-first, the complex-resume ancestor-EXISTS splice and complex-resume watched/progress conjunct, resume-simple, Similar-items, People, Studios, Type-29, and links-search. | Fan-out rewrites enabled. | opt-out | Literal `1` disables the fan-out rewrites. | Unset, literal `0`, and any other value enable the fan-out rewrites. | Both variants; runtime-gated by target DB basename. | Cached once per process. Independent of FTS, dashboard, and `SQLITE3_DISABLE_AUTOPRAGMA`. |
| `SQLITE3_DISABLE_EMBY_DASHBOARD_REWRITE` | kill-switch | Control the Emby Episodes-Latest, movies-Latest, and mixed-Latest dashboard rewrites. | Dashboard rewrites disabled. | opt-in | Literal `0` enables all three dashboard rewrites. | Unset, literal `1`, and any other value disable all three dashboard rewrites. | Both variants; runtime-gated by target DB basename. | Cached once per process. Independent of FTS, fan-out, and `SQLITE3_DISABLE_AUTOPRAGMA`; modes are `dashboard+episodes_latest`, `dashboard+movies_latest`, and `dashboard+mixed_latest`. |
...
| `PLEX_TRIM_FINISHED_SEASON_BLOBS` | kill-switch | Control the fixed 24-month finished-season blob trim within the optional Plex blob database pass. | `0` in the config template; unset behaves as `0`. | opt-in | Literal `1` enables the trim only when `PLEX_PROCESS_BLOB_DB=1`. | Unset, literal `0`, any other value, or a disabled blob database pass skip the trim. | Maintenance script. | Sourced from the Bash config file (`scripts/optimize_media_servers.conf` by default), not via `getenv`; the trim runs as the staged pre-swap hook. |
| `STATS_BANDWIDTH_RETAIN_DAYS` | numeric tunable | Retention window for Plex `statistics_bandwidth` deflate on the staged main database. | `90` in the config template. | tunable | Decimal digits are accepted and used as days. | Empty, unset, or any non-digit value skips the bandwidth deflate with a warning. | Maintenance script. | Sourced from the Bash config file (`scripts/optimize_media_servers.conf` by default), not via `getenv`; DELETE or VACUUM failures warn and continue, and the later final staged integrity gate blocks publication on corruption. |
