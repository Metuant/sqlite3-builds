...
| `tests/rewrite_smoke_harness.h` | Header-only declarative runner shared by the Plex linked, Emby linked, and Emby direct rewrite smokes; owns registry validation, child environment, managed databases, matrix phases, capture, cleanup, and PASS/SKIP rendering. |
| `tests/check_custom_adapter_guard.sh` | Preflight lint for shared-harness custom assertions: enforces the `rsh_custom_adapter_` assignment prefix and rejects runner-owned lifecycle APIs in adapter bodies, with a planted negative demonstration. |
