...
for cmd in cat curl sha256sum awk tar tr uname grep sed; do require_cmd "$cmd"; done

fatal() { echo "FATAL: $*" >&2; exit 1; }

# BEGIN SQLITE_LIBRARY_PINS
SQLITE_LIBRARY_PINS_SCHEMA_VERSION="1"
SQLITE_LIBRARY_PINS_WINDOW_N="5"
SQLITE_LIBRARY_PINS_RELEASE_TAG="unreleased"
SQLITE_LIBRARY_PINS_GENERATED_AT="unreleased"
SQLITE_LIBRARY_PINS=$(cat <<'PINS_EOF'
version|1|managed_window|5|release_tag|unreleased|generated_at|unreleased
PINS_EOF
)
# END SQLITE_LIBRARY_PINS
...
if [[ "${SQLITE_LIBRARY_PINS_RELEASE_TAG}" != "${TAG}" ]]; then
  fatal "deploy script is pinned for ${SQLITE_LIBRARY_PINS_RELEASE_TAG}, but TAG=${TAG}; use the update-sqlite-library.sh asset from the matching ${TAG} release"
fi
...
deploy() {
  ...
  local expected_archive_sha expected_so_sha asserted_sha
  ...
  if [[ -f "${target_path}" ]]; then
    OUT_ASSERTED_SHA=""
    assert_pre_replacement_sha "${target}"
    asserted_sha="${OUT_ASSERTED_SHA}"
  else
  ...
  # WHY: TOCTOU guard -- re-verify target SHA matches asserted value before mutation.
  local toctou_sha
  toctou_sha="$(sha256_of "${target_path}")"
  if [[ "${toctou_sha}" != "${asserted_sha}" ]]; then
    fatal "TOCTOU: ${target_path} changed between assertion and replacement (asserted ${asserted_sha}, now ${toctou_sha})"
  fi
