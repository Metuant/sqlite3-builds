...
static int configure_before_trace_open(const char *label) {
    int rc = sqlite3_config(SQLITE_CONFIG_MULTITHREAD);
    if (rc != SQLITE_OK) {
        fprintf(stderr,
                "FAIL [%s]: SQLITE_CONFIG_MULTITHREAD rc=%d expected=%d\n",
                label, rc, SQLITE_OK);
        return 0;
    }
    ...
    if (rc != SQLITE_OK) {
        fprintf(stderr,
                "FAIL [%s]: SQLITE_CONFIG_MEMSTATUS rc=%d expected=%d\n",
                label, rc, SQLITE_OK);
        return 0;
    }
    return 1;
...
static int run_analysis_limit_open_trace_child_case(
    ...
    if (!configure_before_trace_open(label)) return 0;
...
static int run_named_child_case(const char *case_name) {
    if (strcmp(case_name, "analysis-limit-open-trace-active") == 0) {
        return run_analysis_limit_open_trace_child_case(
            ...
            "/tmp/config_after_dlopen_analysis_limit_active",
            "/tmp/config_after_dlopen_analysis_limit_active/library.db",
            1
        );
    }
    if (strcmp(case_name, "analysis-limit-open-trace-disabled") == 0) {
        return run_analysis_limit_open_trace_child_case(
            ...
            "/tmp/config_after_dlopen_analysis_limit_disabled",
            "/tmp/config_after_dlopen_analysis_limit_disabled/library.db",
            0
        );
    }
...
int main(int argc, char **argv) {
    ...
    if (argc != 1) {
        fprintf(stderr, "usage: %s [--case name|--concurrent-first-open]\n", argv[0]);
        return 1;
    }
