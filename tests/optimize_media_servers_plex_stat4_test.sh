...
assert_contains "$plex_stat4_leaders" "$PLEX_MEDIA_COUNT_INDEX" "STAT4 leader list media-count index"
...
assert_contains "$targets" $'I\tidx_dshadow_metadata_items_section_id_type' "STAT4 worklist media-count metadata_items index target"
...
assert_contains "$stat4_log" 'ANALYZE "idx_dshadow_metadata_items_section_id_type"' "STAT4 run analyzes media-count metadata_items index"
...
if [ "$stat4_available" = "1" ]; then
  ...
  assert_contains "$(cat "$tmp/stat4-run.out")" "Plex STAT4 $PLEX_MEDIA_COUNT_INDEX rows:" "STAT4 run media-count leader progress"
  ...
  media_count_stat4="$("$real_sqlite" "$fixture" "SELECT COUNT(*) FROM sqlite_stat4 WHERE idx='$PLEX_MEDIA_COUNT_INDEX';")"
  ...
  assert_int_gt "$media_count_stat4" 0 "media-count leader index sqlite_stat4 rows"
else
